...
from data.models import Category, CategoryAccess, Topic, User
...
def get_category(
    db: Session, category_id: int, current_user: User = Depends(get_current_user)
):
    ...
    if category.is_private:
        if current_user.role == Roles.admin:
            return category
        access = (
            db.query(CategoryAccess)
            .filter_by(
                category_id=category_id, user_id=current_user.id, read_access=True
            )
            .first()
        )
        if not access:
...
def get_categories(
    ...
    if current_user.role == Roles.admin:
        ...
        categories = (
            ...
            .filter(
                or_(Category.is_private is False, CategoryAccess.read_access is True)
            )
        )
    if search:
...
def toggle_category_visibility(
    ...
    db.commit()
    return {
        "message": f"Visibility for category '{category.name}' changed to {'private' if category.is_private else 'public'}.",
        "category": {
            "id": category.id,
            "name": category.name,
...
def check_if_private(category: Category):
    if not category.is_private:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="The category is public."
        )
...
def read_access(
    ...
    if access_record is None:
        access_record = CategoryAccess(
            category_id=category_id, user_id=user_id, read_access=True
        )
...
def revoke_user_access(
    ...
    if access_record is None or (
        not access_record.read_access and not access_record.write_access
    ):
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
