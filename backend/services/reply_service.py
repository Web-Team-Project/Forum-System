...
def create_reply(db: Session, reply_req: CreateReplyRequest, current_user):
    ...
    if topic.is_locked:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="The topic is locked."
        )
    has_write_access(db, topic.category_id, current_user.id)
    new_reply = Reply(
        content=reply_req.content, user_id=current_user.id, topic_id=reply_req.topic_id
    )
    db.add(new_reply)
    db.commit()
...
def add_best_reply(db: Session, topic_id: int, reply_id: int, user_id: int):
    ...
    reply = (
        db.query(Reply).filter(Reply.id == reply_id, Reply.topic_id == topic_id).first()
    )
    if not reply:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Reply not found."
        )
    db.query(Reply).filter(Reply.topic_id == topic_id).update(
        {Reply.is_best_reply: False}
    )
