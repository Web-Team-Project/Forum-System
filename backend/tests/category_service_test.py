...
class CategoryService_Should(unittest.TestCase):
    ...
    def test_toggle_category_whenWorksCorrectly(self):
        ...
        with patch("services.category_service.get_category") as mock_get_category:
            ...
            service.toggle_category_visibility(category_id, db=mock_db, current_user=mock_current_user)
    ...
    def test_write_access_whenUserIsAdmin(self):
        ...
        result = service.write_access(mock_db, category_id, user_id, current_user=mock_current_user)
    ...
    def test_write_access_raisesError_whenNoAccessGiven(self):
        ...
        mock_db.query().filter_by().first.return_value = mock_access_record
        with self.assertRaises(HTTPException) as context:
            service.write_access(mock_db, category_id, user_id, current_user=mock_current_user)
        self.assertEqual(context.exception.status_code, status.HTTP_403_FORBIDDEN)
        self.assertEqual(context.exception.detail, "The user is not authorized to perform this action.")
    ...
    def test_revoke_user_access_whenReadIsRevoked(self):
        ...
        access_type = "read"
        mock_db.query().filter_by().first.return_value = mock_access_record
        result = service.revoke_user_access(mock_db, category_id, user_id, access_type, current_user=mock_current_user)
    ...
    def test_revoke_user_access_raisesError_whenUserNoPermissions(self):
        ...
        mock_db.query().filter_by().first.return_value = mock_access_record
        with self.assertRaises(HTTPException) as context:
            service.revoke_user_access(mock_db, category_id, user_id, access_type, current_user=mock_current_user)
        self.assertEqual(context.exception.status_code, status.HTTP_404_NOT_FOUND)
    ...
    def test_lock_category_whenItWorksCorrectly(self):
        ...
        result = service.lock_category_for_users(category_id, mock_current_user, mock_db)
