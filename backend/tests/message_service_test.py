...
class MessageService_Should(unittest.TestCase):
    def test_create_message_whenCreatedSuccessfully(self):
        ...
        message_request = CreateMessageRequest(
            text=message_text, receiver_id=receiver_id
        )
        created_message = create_message(
            message_request, current_user=mock_current_user, db=mock_db
        )
    ...
    def test_create_message_whenSenderIsTheSameAsTheReceiver(self):
        ...
        self.assertEqual(
            context.exception.detail, "You cannot send a message to yourself"
        )
    ...
    def test_format_message_whenFormattedCorrectly(self):
        ...
        mock_query.filter().scalar.side_effect = lambda x=None: (
            "sender_username" if "sender_id" in str(x) else "receiver_username"
        )
    ...
    def test_get_conversations_whenValid(self):
        ...
        messages_data = [
            {
                "sender_id": 1,
                "receiver_id": 2,
                "sent_at": datetime(2024, 5, 1, 10, 30),
                "text": "Hello dude",
            },
            {
                "sender_id": 2,
                "receiver_id": 1,
                "sent_at": datetime(2024, 5, 2, 11, 30),
                "text": "Hello bro",
            },
            {
                "sender_id": 3,
                "receiver_id": 1,
                "sent_at": datetime(2024, 5, 3, 12, 30),
                "text": "Hello mate",
            },
        ]
        ...
        with patch("services.message_service.format_message", mock_format_message):
            conversations = get_conversations(
                db=mock_session, current_user_id=current_user_id
            )
    ...
    def test_get_conversation_whenValid(self):
        ...
        with patch("services.message_service.format_message", mock_format_message):
            conversation = get_conversation(
                db=mock_session, current_user_id=current_user_id, user_id=user_id
            )
