...
from data.models import Reply, Topic, Vote
...
class ReplyService_Should(unittest.TestCase):
    def test_create_reply_whenCreatedSuccessfully(self):
        topic_id = 1
        category_id = 1
        user_id = 1
        ...
        new_reply = create_reply(
            db=mock_session, reply_req=reply_req, current_user=mock_current_user
        )
        assert new_reply.content == content

    def test_create_reply_raisesError_whenTopicNotFound(self):
    ...
    def test_create_reply_raisesError_whenTopicIsLocked(self):
        mock_has_write_access = MagicMock()
        mock_has_write_access.return_value = True
    ...
    def test_add_vote_whenNewAddedSuccessfully(self):
        user_id = 1
        reply_id = 1
        vote_type = 1
        ...
        result, message = add_or_update_vote(
            db=mock_session, user_id=user_id, reply_id=reply_id, vote_type=vote_type
        )
    ...
    def test_update_existing_vote_whenUpdatedSuccessfully(self):
        user_id = 1
        reply_id = 1
        vote_type = -1
    ...
    def test_vote_whenVoteAlreadyExists(self):
        user_id = 1
        reply_id = 1
        vote_type = 1
    ...
    def test_reply_raisesError_whenReplyNotFound(self):
        user_id = 1
        reply_id = 1
        vote_type = 1
    ...
    def test_add_best_reply_raisesError_whenReplyNotFound(self):
        topic_id = 1
        reply_id = 1
        user_id = 1
        ...
        try:
            add_best_reply(
                db=mock_session, topic_id=topic_id, reply_id=reply_id, user_id=user_id
            )
        except HTTPException as e:
            assert e.status_code == status.HTTP_404_NOT_FOUND
            assert e.detail == "Topic not found."

    def test_add_best_reply_raisesError_whenUserUnauthorized(self):
        topic_id = 1
        reply_id = 1
        user_id = 1
        ...
        mock_session = MagicMock(spec=Session)
        mock_topic = MagicMock(spec=Topic)
        mock_topic.author_id = 2
        mock_session.query(Topic).filter(
            Topic.id == topic_id
        ).first.return_value = mock_topic
        try:
            add_best_reply(
                db=mock_session, topic_id=topic_id, reply_id=reply_id, user_id=user_id
            )
    ...
    def test_add_best_reply_raisesError_whenTopicNotFound(self):
        topic_id = 1
        reply_id = 1
        user_id = 1
        ...
        mock_session.query(Reply).filter(
            Reply.id == reply_id, Reply.topic_id == topic_id
    ...
    def test_add_best_reply_whenAddedSuccessfully(self):
        topic_id = 1
        reply_id = 1
        user_id = 1
        ...
        assert (
            mock_session.query(Topic).filter(Topic.id == topic_id).first() == mock_topic
        )
        assert (
            mock_session.query(Reply)
            .filter(Reply.id == reply_id, Reply.topic_id == topic_id)
            .first()
            == mock_reply
        )
        try:
            result = add_best_reply(
                db=mock_session, topic_id=topic_id, reply_id=reply_id, user_id=user_id
            )
